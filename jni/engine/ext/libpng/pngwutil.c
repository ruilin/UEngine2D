...
#ifdef PNG_FIXED_POINT_SUPPORTED
void /* PRIVATE */
png_write_sCAL_s(png_structp png_ptr, int unit, png_charp screenWidth,
   png_charp screenHeight)
{
   PNG_sCAL;
   png_byte buf[64];
   ...
   wlen = png_strlen(screenWidth);
   hlen = png_strlen(screenHeight);
   ...
   buf[0] = (png_byte)unit;
   png_memcpy(buf + 1, screenWidth, wlen + 1);      /* Append the '\0' here */
   png_memcpy(buf + wlen + 2, screenHeight, hlen);  /* Do NOT append the '\0' here */
